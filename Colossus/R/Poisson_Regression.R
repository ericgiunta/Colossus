...
#    a_n <- c(a_n, runif(length(val$cols),min=-0.01,0.01))
    ...
    e <- poisson_strata_transition(as.matrix(df[,ce, with = FALSE]),Term_n,tform,a_n,dfc,x_all, fir,der_iden,
        modelform, control,keep_constant,term_tot,as.matrix(df0[,val$cols, with=FALSE]))
    return (e)
}
...
#    a_n <- c(a_n, runif(length(val$cols),min=-0.01,0.01))
    ...
    #
    e <- poisson_strata_transition_single(as.matrix(df[,ce, with = FALSE]),Term_n,tform,a_n,dfc,x_all, fir,
        modelform, control,keep_constant,term_tot,as.matrix(df0[,val$cols, with=FALSE]))
    return (e)
}
...
#        a_n <- c(a_n, runif(length(val$cols),min=-0.01,0.01))
        ...
        e <- poisson_strata_transition(as.matrix(df[,ce, with = FALSE]),Term_n,tform,a_n,dfc,x_all, fir,der_iden,
             modelform, control,keep_constant,term_tot,as.matrix(df0))
        if (is.na(e$Deviation)){
            #fine
        }
        for (i in seq_len(length(e$beta_0))){
...
#' @importFrom rlang .data
RunPoissonRegression_Guesses_CPP <- function(df, pyr, event0, names, Term_n, tform, keep_constant, a_n, modelform, fir, der_iden, control, guesses_control,Strat_Col){
    if (typeof(a_n)!="list"){
        a_n <- list(a_n)
    }
    control <- Def_Control(control)
    ...
    if (guesses_control$strata==FALSE){
        ...
        #
        e <- poisson_transition_guess(as.matrix(df[,ce, with = FALSE]),Term_n,tform,matrix(a_ns,nrow=length(control$maxiters)-1,byrow=T),dfc,x_all, fir,der_iden, modelform, control,keep_constant,term_tot)
        #fine
        return (e)
    } else {
        #
        ...
        control$guesses <- length(maxiters)-1
        #
        e <- poisson_transition_strata_guess(as.matrix(df[,ce, with = FALSE]),Term_n,tform,matrix(a_ns,nrow=length(control$maxiters)-1,byrow=T),dfc,x_all, fir,der_iden, modelform, control,keep_constant,term_tot,as.matrix(df0[,val$cols, with=FALSE]))
        #fine
        return (e)
    }
    return (NULL)
