...
//    Rcout << dfc << endl;
//    Rcout << fir << endl;
//    Rcout << T0.rows() << " " << T0.cols() << endl;
//    Rcout << Td0.rows() << " " << Td0.cols() << endl;
//    Rcout << Tdd0.rows() << " " << Tdd0.cols() << endl;
//    Rcout << Dose.rows() << " " << Dose.cols() << endl;
//    Rcout << nonDose.rows() << " " << nonDose.cols() << endl;
//    Rcout << TTerm.rows() << " " << TTerm.cols() << endl;
//    Rcout << nonDose_LIN.rows() << " " << nonDose_LIN.cols() << endl;
//    Rcout << nonDose_PLIN.rows() << " " << nonDose_PLIN.cols() << endl;
//    Rcout << nonDose_LOGLIN.rows() << " " << nonDose_LOGLIN.cols() << endl;
//    Rcout << beta_0.size() << endl;
//    Rcout << df0.rows() << " " << df0.cols() << endl;
//    Rcout << dint << endl;
//    Rcout << dslp << endl;
//    Rcout << nthreads << endl;
//    Rcout << debugging << endl;
...
//    MatrixXd Lldd_inv = -1 * Lldd_mat.inverse().matrix(); //uses inverse information matrix to calculate the standard deviation
    //
    List res_list = List::create(_["LogLik"]=wrap(Ll[0]),_["First_Der"]=wrap(Lld),_["Second_Der"]=Lldd_vec,_["beta_0"]=wrap(beta_0) ,_["Standard_Deviation"]=wrap(stdev) ,_["AIC"]=2*(totalnum-accumulate(KeepConstant.begin(),KeepConstant.end(), 0.0))-2*Ll[0],_["Parameter_Lists"]=para_list,_["Control_List"]=control_list);
...
//    MatrixXd Lldd_inv = -1 * Lldd_mat.inverse().matrix(); //uses inverse information matrix to calculate the standard deviation
    //
    List res_list = List::create(_["LogLik"]=wrap(Ll[0]),_["First_Der"]=wrap(Lld),_["Second_Der"]=Lldd_vec,_["beta_0"]=wrap(beta_0) ,_["Standard_Deviation"]=wrap(stdev) ,_["AIC"]=2*(totalnum-accumulate(KeepConstant.begin(),KeepConstant.end(), 0.0))-2*Ll[0],_["Control_List"]=control_list);
    // returns a list of results
    return res_list;
}
...
List LogLik_Poisson( MatrixXd PyrC, IntegerVector Term_n, StringVector tform, NumericVector a_n,NumericMatrix x_all,IntegerVector dfc,int fir, int der_iden,string modelform, double lr, int maxiter, int halfmax, double epsilon, double dbeta_cap, double abs_max,double dose_abs_max, double deriv_epsilon, int double_step,bool change_all, bool verbose, bool debugging, IntegerVector KeepConstant, int term_tot){
    ...
    // ---------------------------------------------------------
//    for (int ijk=0;ijk<totalnum;ijk++){
//        Rcout << beta_0[ijk] << endl;
//        Rcout << T0.col(ijk).transpose() << endl;
//        Rcout << df0.col(dfc[ijk]-1).transpose() << endl;
//    }
//    Rcout << "temp" << endl;
    if (verbose){
        Rcout << "values checked ";
...
//    MatrixXd Lldd_inv = -1 * Lldd_mat.inverse().matrix(); //uses inverse information matrix to calculate the standard deviation
    //
    List res_list = List::create(_["LogLik"]=wrap(Ll[0]),_["First_Der"]=wrap(Lld),_["Second_Der"]=Lldd_vec,_["beta_0"]=wrap(beta_0) ,_["Standard_Deviation"]=wrap(stdev) ,_["AIC"]=2*(totalnum-accumulate(KeepConstant.begin(),KeepConstant.end(), 0.0))+dev,_["Deviation"]=dev,_["Parameter_Lists"]=para_list,_["Control_List"]=control_list);
    // returns a list of results
    return res_list;
}
...
//                Rcout << "(" << ij <<"," << jk << ") (" << pij_ind << "," << pjk_ind << ") (" << KeepConstant[ij] << "," << KeepConstant[jk] << ") " << Lldd[ij*totalnum+jk] << endl;
    ...
    for (int ijk=0;ijk<true_totalnum*(true_totalnum+1)/2;ijk++){
        ...
        Lldd_vec[ij * true_totalnum + jk]=Lldd_vec[jk * true_totalnum + ij];
    }
    //
...
;
    ...
    if (Debug_It[0]){
            Make_Subterms( totalnum, Term_n, tform, dfc, fir, T0, Td0, Tdd0, Dose, nonDose, TTerm,  nonDose_LIN, nonDose_PLIN, nonDose_LOGLIN ,beta_0, df0,dint,dslp,nthreads, TRUE);
    } else {
            Make_Subterms( totalnum, Term_n, tform, dfc, fir, T0, Td0, Tdd0, Dose, nonDose, TTerm,  nonDose_LIN, nonDose_PLIN, nonDose_LOGLIN ,beta_0, df0,dint,dslp,nthreads, FALSE);
    }
    // ---------------------------------------------------------
    // Prints off a series of calculations to check at what point values are changing
    // ---------------------------------------------------------    
...
NumericVector RISK_SUBSET(IntegerVector Term_n, StringVector tform, NumericVector a_n,NumericMatrix x_all,IntegerVector dfc,int fir,string modelform, bool verbose, bool debugging, int term_tot){
    ...
    // Calculates terms
    //
    Make_Subterms( totalnum, Term_n, tform, dfc, fir, T0, Td0, Tdd0, Dose, nonDose, TTerm,  nonDose_LIN, nonDose_PLIN, nonDose_LOGLIN ,beta_0, df0,1.0,1.0,nthreads, debugging);
    MatrixXd RdR = MatrixXd::Zero(df0.rows(), totalnum); //preallocates matrix for Risk to derivative ratios
    MatrixXd RddR = MatrixXd::Zero(df0.rows(), totalnum*(totalnum+1)/2);
    //
    // Calculates risk
