...
//' Primary Cox PH regression with STRATA effect and multiplicative log-linear model
//' \code{LogLik_Cox_PH_STRATA_BASIC} Performs the calls to calculation functions, Structures the Cox PH regression, With verbose option prints out time stamps and intermediate sums of terms and derivatives
//'
//' @param     a_n    starting values
//' @param     x_all    covariate matrix
//' @param     dfc    covariate column numbers
...
//' @return List of results: Log-likelihood of optimum, first derivative of log-likelihood, second derivative matrix, parameter list, standard deviation estimate, AIC, model information
// [[Rcpp::export]]
List LogLik_Cox_PH_STRATA_BASIC( NumericVector a_n,NumericMatrix x_all,IntegerVector dfc, double lr, int maxiter, int halfmax, double epsilon, double dbeta_cap, double abs_max,double dose_abs_max, double deriv_epsilon, NumericMatrix df_groups, NumericVector tu, int double_step ,bool change_all, bool verbose, bool debugging, IntegerVector KeepConstant, string ties_method, NumericVector& STRATA_vals, int nthreads){
    ;
    //
    List temp_list = List::create(_["Status"]="FAILED"); //used as a dummy return value for code checking
    if (verbose){
...
//' @return List of results: Log-likelihood of optimum, first derivative of log-likelihood, second derivative matrix, parameter list, standard deviation estimate, AIC, deviance, model information
// [[Rcpp::export]]
List LogLik_Poisson_STRATA( MatrixXd PyrC, IntegerVector Term_n, StringVector tform, NumericVector a_n,NumericMatrix x_all,IntegerVector dfc,int fir, int der_iden,string modelform, double lr, int maxiter, int halfmax, double epsilon, double dbeta_cap, double abs_max,double dose_abs_max, double deriv_epsilon, int double_step,bool change_all, bool verbose, bool debugging, IntegerVector KeepConstant, int term_tot, const MatrixXd& dfs, int nthreads){
    ;
    //
    List temp_list = List::create(_["Status"]="FAILED"); //used as a dummy return value for code checking
    if (verbose){
...
//' Primary Cox PH regression with STRATA effect and calculating only the Log-Likelihood
//' \code{LogLik_Cox_PH_STRATA_SINGLE} Performs the calls to calculation functions, Structures the Cox PH regression, With verbose option prints out time stamps and intermediate sums of terms and derivatives
//'
//' @param     Term_n    Term numbers
//' @param     tform    subterm types
//' @param     a_n    starting values
...
//' @return List of results: Log-likelihood of optimum, first derivative of log-likelihood, second derivative matrix, parameter list, standard deviation estimate, AIC, model information
// [[Rcpp::export]]
List LogLik_Cox_PH_STRATA_SINGLE( IntegerVector Term_n, StringVector tform, NumericVector a_n,NumericMatrix x_all,IntegerVector dfc,int fir,string modelform, NumericMatrix df_groups, NumericVector tu, bool verbose, bool debugging, IntegerVector KeepConstant, int term_tot, string ties_method, NumericVector& STRATA_vals, int nthreads){
    ;
    //
    List temp_list = List::create(_["Status"]="FAILED"); //used as a dummy return value for code checking
    if (verbose){
        Rcout << "START_COX_STRATA_SINGLE" << endl;
    }
    time_point<system_clock> start_point, end_point;
    start_point = system_clock::now();
    auto start = time_point_cast<microseconds>(start_point).time_since_epoch().count();
    ...
    //
    List res_list = List::create(_["LogLik"]=wrap(Ll[0]),_["beta_0"]=wrap(beta_0) ,_["AIC"]=2*(totalnum-accumulate(KeepConstant.begin(),KeepConstant.end(), 0.0))-2*Ll[0]);
    // returns a list of results
    return res_list;
}
...
//' Primary poisson regression function with strata effect and no derivatives calculated
//' \code{LogLik_Poisson_STRATA_SINGLE} Performs the calls to calculation functions, Structures the poisson regression, With verbose option prints out time stamps and intermediate sums of terms and derivatives, all with strata effects
//'
//' @param     PyrC    person-year matrix
//' @param     Term_n    Term numbers
//' @param     tform    subterm types
...
//' @return List of results: Log-likelihood of optimum, first derivative of log-likelihood, second derivative matrix, parameter list, standard deviation estimate, AIC, deviance, model information
// [[Rcpp::export]]
List LogLik_Poisson_STRATA_SINGLE( MatrixXd PyrC, IntegerVector Term_n, StringVector tform, NumericVector a_n,NumericMatrix x_all,IntegerVector dfc,int fir,string modelform, bool verbose, bool debugging, IntegerVector KeepConstant, int term_tot, const MatrixXd& dfs, int nthreads){
    ;
    //
    List temp_list = List::create(_["Status"]="FAILED"); //used as a dummy return value for code checking
    if (verbose){
        Rcout << "START_POISSON_STRATA_SINGLE" << endl;
    }
    time_point<system_clock> start_point, end_point;
    start_point = system_clock::now();
    auto start = time_point_cast<microseconds>(start_point).time_since_epoch().count();
    ...
    //
    // Calculates the risk for each row
    Make_Risks_Weighted_Single(modelform, tform, Term_n, totalnum, fir, s_weights, T0, Te, R, Dose, nonDose, TTerm, nonDose_LIN, nonDose_PLIN, nonDose_LOGLIN, nthreads, debugging,KeepConstant);
    //
    //
    //
    if (verbose){
...
//' Primary Cox PH regression with STRATA effect, calculating only the Log-Likelihood, and competing risks event=2
//' \code{LogLik_Cox_PH_STRATA_SINGLE_CR} Performs the calls to calculation functions, Structures the Cox PH regression, With verbose option prints out time stamps and intermediate sums of terms and derivatives
//'
//' @param     Term_n    Term numbers
//' @param     tform    subterm types
//' @param     a_n    starting values
...
//' @return List of results: Log-likelihood of optimum, first derivative of log-likelihood, second derivative matrix, parameter list, standard deviation estimate, AIC, model information
// [[Rcpp::export]]
List LogLik_Cox_PH_STRATA_SINGLE_CR( IntegerVector Term_n, StringVector tform, NumericVector a_n,NumericMatrix x_all,IntegerVector dfc,int fir,string modelform, NumericMatrix df_groups, NumericVector tu, const VectorXd cens_weight, const double cens_thres, bool verbose, bool debugging, IntegerVector KeepConstant, int term_tot, string ties_method, NumericVector& STRATA_vals, int nthreads){
    ;
    //
    List temp_list = List::create(_["Status"]="FAILED"); //used as a dummy return value for code checking
    if (verbose){
        Rcout << "START_COX_STRATA_SINGLE_CR" << endl;
    }
    time_point<system_clock> start_point, end_point;
    start_point = system_clock::now();
    auto start = time_point_cast<microseconds>(start_point).time_since_epoch().count();
    ...
    const Map<MatrixXd> df_m(as<Map<MatrixXd> >(df_groups));
    // Creates matrices used to identify the event risk groups
    Make_Groups_STRATA_CR( ntime, df_m, RiskFail, RiskGroup, tu, nthreads, debugging,STRATA_vals,cens_weight,cens_thres);
    //
    if (verbose){
        end_point = system_clock::now();
        ending = time_point_cast<microseconds>(end_point).time_since_epoch().count();
...
//' Primary Cox PH regression with STRATA effect and competing risks, event=2
//' \code{LogLik_Cox_PH_STRATA_CR} Performs the calls to calculation functions, Structures the Cox PH regression, With verbose option prints out time stamps and intermediate sums of terms and derivatives
//'
//' @param     Term_n    Term numbers
//' @param     tform    subterm types
//' @param     a_n    starting values
...
//' @return List of results: Log-likelihood of optimum, first derivative of log-likelihood, second derivative matrix, parameter list, standard deviation estimate, AIC, model information
// [[Rcpp::export]]
List LogLik_Cox_PH_STRATA_CR( IntegerVector Term_n, StringVector tform, NumericVector a_n,NumericMatrix x_all,IntegerVector dfc,int fir, int der_iden,string modelform, double lr, int maxiter, int halfmax, double epsilon, double dbeta_cap, double abs_max,double dose_abs_max, double deriv_epsilon, NumericMatrix df_groups, NumericVector tu, const VectorXd cens_weight, const double cens_thres, int double_step ,bool change_all, bool verbose, bool debugging, IntegerVector KeepConstant, int term_tot, string ties_method, NumericVector& STRATA_vals, int nthreads){
    ;
    //
    List temp_list = List::create(_["Status"]="FAILED"); //used as a dummy return value for code checking
    if (verbose){
        Rcout << "START_COX_STRATA_CR" << endl;
    }
    time_point<system_clock> start_point, end_point;
    start_point = system_clock::now();
    auto start = time_point_cast<microseconds>(start_point).time_since_epoch().count();
...
//' Primary Cox PH regression with competing risks and no derivative calculation, event=2
//' \code{LogLik_Cox_PH_CR_SINGLE} Performs the calls to calculation functions, Structures the Cox PH regression with competing events weighted by censoring rates, With verbose option prints out time stamps and intermediate sums of terms
//'
//' @param     Term_n    Term numbers
//' @param     tform    subterm types
//' @param     a_n    starting values
...
//' @return List of results: Log-likelihood of optimum, first derivative of log-likelihood, second derivative matrix, parameter list, standard deviation estimate, AIC, model information
// [[Rcpp::export]]
List LogLik_Cox_PH_CR_SINGLE( IntegerVector Term_n, StringVector tform, NumericVector a_n,NumericMatrix x_all,IntegerVector dfc,int fir,string modelform, NumericMatrix df_groups, NumericVector tu, const VectorXd cens_weight, const double cens_thres, bool verbose, bool debugging, IntegerVector KeepConstant, int term_tot, string ties_method, int nthreads){
    ;
    //
    List temp_list = List::create(_["Status"]="FAILED"); //used as a dummy return value for code checking
    if (verbose){
        Rcout << "START_NEW_SINGLE_COMPETING RISKS" << endl;
    }
    time_point<system_clock> start_point, end_point;
    start_point = system_clock::now();
    auto start = time_point_cast<microseconds>(start_point).time_since_epoch().count();
...
//' Primary poisson regression with multiple starting points
//' \code{LogLik_Poisson_Guess} Performs the calls to calculation functions, Structures the poisson regression, With verbose option prints out time stamps and intermediate sums of terms and derivatives
//'
//' @param     PyrC    person-year matrix
//' @param     Term_n    Term numbers
//' @param     tform    subterm types
...
//' @return List of final results: Log-likelihood of optimum, first derivative of log-likelihood, second derivative matrix, parameter list, standard deviation estimate, AIC, model information
// [[Rcpp::export]]
List LogLik_Poisson_Guess(MatrixXd PyrC, IntegerVector Term_n, StringVector tform, NumericMatrix a_ns,NumericMatrix x_all,IntegerVector dfc,int fir, int der_iden,string modelform, double lr, NumericVector maxiters, int guesses, int halfmax, double epsilon, double dbeta_cap, double abs_max,double dose_abs_max, double deriv_epsilon, int double_step ,bool change_all, bool verbose, bool debugging, IntegerVector KeepConstant, int term_tot, int nthreads){
    ;
    //
    List temp_list = List::create(_["Status"]="FAILED"); //used as a dummy return value for code checking
    if (verbose){
        Rcout << "START_POISSON_GUESS" << endl;
    }
    time_point<system_clock> start_point, end_point;
    start_point = system_clock::now();
    auto start = time_point_cast<microseconds>(start_point).time_since_epoch().count();
    ...
    for (int guess=0; guess<=guesses;guess++){
        ...
        RdR = MatrixXd::Zero(df0.rows(), reqrdnum);
        RddR = MatrixXd::Zero(df0.rows(), reqrdnum*(reqrdnum+1)/2);
        dev_temp = MatrixXd::Zero(PyrC.rows(),2);
        dev=0;
        fill(Ll.begin(), Ll.end(), 0.0);
        fill(Lld.begin(), Lld.end(), 0.0);
        fill(Lldd.begin(), Lldd.end(), 0.0);
...
//' Primary poisson regression with multiple starting points with stratification
//' \code{LogLik_Poisson_Strata_Guess} Performs the calls to calculation functions, Structures the poisson regression, With verbose option prints out time stamps and intermediate sums of terms and derivatives
//'
//' @param     PyrC    person-year matrix
//' @param     Term_n    Term numbers
//' @param     tform    subterm types
...
//' @return List of final results: Log-likelihood of optimum, first derivative of log-likelihood, second derivative matrix, parameter list, standard deviation estimate, AIC, model information
// [[Rcpp::export]]
List LogLik_Poisson_Strata_Guess(MatrixXd PyrC, IntegerVector Term_n, StringVector tform, NumericMatrix a_ns,NumericMatrix x_all,IntegerVector dfc,int fir, int der_iden,string modelform, double lr, NumericVector maxiters, int guesses, int halfmax, double epsilon, double dbeta_cap, double abs_max,double dose_abs_max, double deriv_epsilon, int double_step ,bool change_all, bool verbose, bool debugging, IntegerVector KeepConstant, int term_tot, const MatrixXd& dfs, int nthreads){
    ;
    //
    List temp_list = List::create(_["Status"]="FAILED"); //used as a dummy return value for code checking
    if (verbose){
        Rcout << "START_POISSON_STRATA_GUESS" << endl;
    }
    time_point<system_clock> start_point, end_point;
    start_point = system_clock::now();
    auto start = time_point_cast<microseconds>(start_point).time_since_epoch().count();
