test_that("Coxph strata_basic_single_null", {
    ...
    control=list("Ncores"=2,'lr' = 0.75,'maxiters' = c(1,1),'halfmax' = 5,'epsilon' = 1e-6,'dbeta_max' = 0.5,'deriv_epsilon' = 1e-6, 'abs_max'=1.0,'change_all'=TRUE,'dose_abs_max'=100.0,'verbose'=TRUE, 'ties'='breslow','double_step'=1)
...
test_that("Pois strata_single", {
    ...
    for (i in c(TRUE,FALSE)){
        for (j in c(TRUE,FALSE)){
            ...
            if (verbose){print(model_control)}
            a_n <- c(0.01)
            expect_no_error(RunPoissonRegression_Omnibus(df, pyr, event, names, Term_n, tform, keep_constant, a_n, modelform, fir, der_iden, control,Strat_Col))
            if (verbose){print("---------------")}
        }
    }
})
