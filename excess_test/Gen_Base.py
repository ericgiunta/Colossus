...
if (True):
    ...
    psdf0 = psdf0.withColumn("annual_dose", when(psdf0.age_entry<65, 1/60).otherwise(0))
...
#    psdf0.select('UserID','seq_id','age_entry','age_exit','sex','dose').write.parquet('5050.parquet','overwrite')
    ...
    paras=[-0.95, 0.1]
    seed=12132024
    psdf0=psdf0.withColumn("back_rate", exp(-0.03*psdf0.age_entry)*pow(psdf0.age_entry,2)*(1/700000))
...
#    print("-------------------------------------------------------------------------6")
#    print(time.time()-tstart)
    psdf0.select('UserID','seq_id','age_entry','exit_adjust','sex','dose','event').toPandas().to_csv("excess_test.csv", header=True)
    print("-------------------------------------------------------------------------7")
    print(time.time()-tstart)
