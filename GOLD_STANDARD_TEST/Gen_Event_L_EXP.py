...
def Write_Write_L_EXP_Event_Event(iname,oname,paras,seed=3742621):
    tstart = time.time()
    ...
    b=paras[8]
    #
    psdf0=psdf0.withColumn("cov_rate", psdf0.cov_rate*(lit(1)+a*psdf0.dose)*exp(b*pow(psdf0.dose,2)))
...
for i_f in ["5050","7525","2575"]:
    for i in [1,2,4,5]:
        if i==1:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, -0.1/30, -0.005]
        elif i==2:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, -0.01/30, -0.0005]
        elif i==4:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, 0.25/30, -0.0001]
        elif i==5:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, 3/30, -0.0005]
        iname="{}.parquet".format(i_f)
        oname="{}_L_EXP_4405{}.parquet".format(i_f,i)
        print(oname)
        if os.path.exists(oname):
            print("exists")
        else:
            pass
            Write_Write_L_EXP_Event_Event(iname,oname,paras)
