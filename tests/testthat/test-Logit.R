...
test_that("basic regression with link non-fail", {
  if (system.file(package = "survival") != "") {
    ...
    expect_equal(e$LogLik, -32.48003, tolerance = 1e-3)
    expect_no_error(print(e))
    e <- LogisticRun(model, df, control = control, link = "loglink", a_n = a_n)
    expect_equal(e$LogLik, -97.8412, tolerance = 1e-3)
    expect_no_error(print(e))
    e <- LogisticRun(model, df, control = control, link = "probit", a_n = a_n)
    expect_equal(e$LogLik, -31.06014, tolerance = 1e-3)
    expect_no_error(print(e))
    sink(type = "message")
    sink(NULL)
    close(zz)
...
test_that("epicure check", {
  ...
  e <- LogisticRun(model, df, control = control, a_n = a_n)
  expect_equal(e$beta_0, c(0.09883952, 0.01229789, 0.11170645), tolerance = 1e-3)
  ...
  model <- logit(n, x) ~ linear(CONST, factor(alcohol))
  e <- LogisticRun(model, df, control = control, a_n = a_n, link = "loglink")
  expect_equal(e$beta_0, c(2.4087301, -0.1061499, -0.6595241), tolerance = 1e-3)

  a_n <- c(0.01, 0.01, 0.01)
  model <- logit(n, x) ~ linear(CONST, factor(alcohol))
  e <- LogisticRun(model, df, control = control, a_n = a_n, link = "ident")
  expect_equal(e$beta_0, c(0.08989964, 0.01009402, 0.08397130), tolerance = 1e-3)
})
