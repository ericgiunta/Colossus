...
def Write_LL_Event(iname,oname,paras,seed=3742621):
    ...
    psdf0=psdf0.withColumn("exit_adjust", when(psdf0.age_exit>psdf0.exit_adjust,psdf0.exit_adjust).otherwise(psdf0.age_exit))
    ##
    psdf0=psdf0.withColumn("event_t",when(psdf0.event==1,psdf0.age_exit).otherwise(100))
    ##
    w = Window.partitionBy('UserID')
    psdf0 = psdf0.withColumn('minEt', F.min('event_t').over(w))
    psdf0=psdf0.withColumn("event", when(psdf0.minEt<psdf0.age_exit,2).otherwise(psdf0.event))
    ##
    print("-------------------------------------------------------------------------3")
    print(time.time()-tstart)
...
for i_f in ["5050","2575","7525"]:
    for i in [1,2,3,4,5]:
        if i==1:
            paras=[-0.69,0.1,0.69,0.1,-0.1,-0.69,0.6,5.0/30.0]
        elif i==2:
            paras=[-0.1,0.1,0.69,0.1,-0.1,-0.69,0.6,5.0/30.0]
        elif i==3:
            paras=[0.0,0.1,0.69,0.1,-0.1,-0.69,0.6,5.0/30.0]
        elif i==4:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6,5.0/30.0]
        elif i==5:
            paras=[0.69,0.1,0.69,0.1,-0.1,-0.69,0.6,5.0/30.0]
        iname="{}.parquet".format(i_f)
        oname="{}_LL_{}4055.parquet".format(i_f,i)
        print(oname)
