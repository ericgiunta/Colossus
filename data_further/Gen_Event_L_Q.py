...
def Write_L_Q_Event(iname,oname,paras,seed=3742621):
    tstart = time.time()
    ...
    b=paras[8]
    #
    psdf0=psdf0.withColumn("cov_rate", psdf0.cov_rate*(lit(1)+a*psdf0.dose + b*pow(psdf0.dose,2)))
...
for i_f in ["5050","7525","2575"]:
    for i in [1,2,4,5]:
        if i==1:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, -0.5/30, -0.015/30]
        elif i==2:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, -0.05/30, -0.01/30]
        elif i==4:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, 0.05/30, 0.01/30]
        elif i==5:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, 0.5/30, 0.05/30]
        iname="{}.parquet".format(i_f)
        oname="{}_L_Q_4405{}.parquet".format(i_f,i)
        print(oname)
        if os.path.exists(oname):
            print("exists")
        else:
            pass
            Write_L_Q_Event(iname,oname,paras)
