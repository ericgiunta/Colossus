...
def Write_Q_Event(iname,oname,paras,seed=3742621):
    tstart = time.time()
    ...
    psdf0=psdf0.withColumn("cov_rate", psdf0.cov_rate*(lit(1) + paras[7]*pow(psdf0.dose,2)))
...
for i_f in ["5050","7525","2575"]:
    for i in [1,2,4,5]:
        if i==1:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, -1*(0.9/30**2)]
        elif i==2:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, -1*(0.15/30**2)]
        elif i==4:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, 1*(0.15/30**2)]
        elif i==5:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, 1*(1.4/30**2)]
        iname="{}.parquet".format(i_f)
        oname="{}_Q_4405{}.parquet".format(i_f,i)
        print(oname)
        if os.path.exists(oname):
            print("exists")
        else:
            pass
            Write_Q_Event(iname,oname,paras)
