...
| epsilon  | Maximum step size before regression terminates. Defaults to 1e-4. Regression exits if the predicted step sizes are all below this threshold.  |
| deriv_epsilon  | Largest maximum first derivative before convergence is assumed. Defaults to 1e-4. |
| ll_epsilon  | Largest maximum change in log-likelihood before convergence is assumed. The threshold is only checked when an iteration finds a new optimum. Defaults to 1e-4. |
...
The following options can be used by the multiple dose realization functions to select how realizations are combined:

| Option | Description |
| :---------: | :----------: |
| fma  | Boolean to select frequentist model averaging, and process realizations in serial. Defaults to FALSE.   |
| mcml  | Boolean to select Monte Carlo maximum likelihood, and optimize the average score over all realizations. Defaults to True.   |
