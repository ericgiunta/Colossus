...
In general the [plot function](../reference/plot.coxres.html) can be used to call the various different types of plots using a list of plotting options. In this vignette we will be calling the specific functions for each application.
...
We can calculate the instantaneous hazard, cumulative hazard, and baseline surviving fraction using the [plotSurvival](../reference/plotSurvival.coxres.html) function. We can get the Kaplan-Meier estimates using the KM option.
...
We can plot schoenfeld residuals using the [plotSchoenfeld](../reference/plotSchoenfeld.coxres.html) function.
...
We can plot martingale residuals using the [plotMartingale](../reference/plotMartingale.coxres.html) function.
...
There is a similar option for returning predicted hazard ratio for a set of input data points, or a reference point, called the [RelativeRisk](../reference/RelativeRisk.coxres.html) function. This can be used similarly to the [plotRisk](../reference/plotRisk.coxres.html) function by manually defining the points to calculate risk at. This has one significant benefit, it can account for interactions. If a model transforms a column, then it has multiple covariates dependent on a single input column. The [plotRisk](../reference/plotRisk.coxres.html) function will only calculate the change in risk based on the final calculated covariates independently, while the [RelativeRisk](../reference/RelativeRisk.coxres.html) function will properly account for how each value depends on each other.
...
x_all <- c(res_age$x, res_age2$x, df_off$age)
