...
e1 <- CoxRun(Cox(time, status) ~ loglinear(temperature, voltage, 0), a_n = a_n, control = control)
print(e1, 5)

e2 <- CoxRun(Cox(time, status) ~ loglinear(temperature, 0) + plinear(voltage, 0), a_n = a_n, control = control)
print(e2, 5)
```
...
curve_control <- list("maxstep" = 20, "alpha" = 0.005, "para_number" = 2, "step_size" = 0.5, "bisect" = TRUE)
