...
function of a single covariate ($x$) and several parameters
($\alpha,\beta$).
...
Note that the multiplicative model is taking the product of terms, each
of which is the product of subterms. The same equation may be written
with subterms distributed over different terms in the multiplicative
model without changing the final value. The only exception is subterms
moved from or into the “first” term in the multiplicative excess model.
The same equation could be expressed in ways with different
...
formula. Every model except the additive model has a distinction between
...
*names* denotes the column names used, *term_n* denotes the term
numbers, *tform* denotes the subterm formula, *modelform* denotes the
term formula, *fir* denotes the “first” term indexed from zero which
defaults to 0, and *a_n* denotes the initial guesses for the parameter
values. All of which are assumed to be in the same order. A function is
called before regression that reorders the inputs in order of terms,
subterm types, etc.
...
always be the case. Colossus is designed to allow the user to input
“%trunc%” in place of a missing side of the interval. Colossus assumes
that this means that the person has the missing endpoint outside of the
available data range and creates a dummy column to reference as the
interval endpoint. So in code, these variables would look like this:
...
column of interval lengths. In this case, we need to define a new
column, but that may not be the case in general. Suppose we are now
working with the following table:
...
are used. The following options are equivalent:
...
The convergence parameters currently supported in general are as
follows:
...
| thres_step_max |                                        Largest starting step size for threshold parameters. Assumes the threshold parameters may be orders of magnitude different than other parameters                                        |
...
The additional information parameters currently supported are as
follows:
...
convergence and information. They are as follows:
...
In this example, no parameters are held constant, 100 iterations with 5
half-steps are used, a regression is performed with matrix step
...
options not covered in this vignette.
...
#> 1:         a  loglin           0         40.29492            NaN            NaN
#> 2:         b     lin           1         40.72266      3322.3606      0.9902205
#> 3:         c     lin           1         38.82311       628.5925      0.9507524
#> 4:         d    plin           2         43.10000       588.7424      0.9416415
#> 
#> Cox Model Used
#> -2*Log-Likelihood: 1.4,  AIC: 9.4
#> Iterations run: 42
#> maximum step size: 1.00e+00, maximum first derivative: 9.82e-04
#> Analysis converged
...
#>    Covariate Subterm Term Number Central Estimate Standard Error 2-tail p-value
#>       <char>  <char>       <int>            <num>          <num>          <num>
#> 1:         a  loglin           0      -0.40680015    0.381634898   2.864505e-01
#> 2:         b     lin           1      -0.01846667    0.035174231   5.995792e-01
#> 3:         c     lin           1      -0.07585910    0.007685371   5.583838e-23
#> 4:         d    plin           2      -0.70133825    0.405384920   8.362043e-02
#> 
#> Poisson Model Used
#> -2*Log-Likelihood: 89.48,  Deviation: 85.48,  AIC: 93.48,  BIC: 97.26
#> Iterations run: 7
#> maximum step size: 9.77e-04, maximum first derivative: 2.31e+03
...
For both cases, the regressions did not converge. Given that the
...
The Poisson model regression exited due to the step limit being below
the threshold, which means that it hit some local optimum. To find a
better solution the user would likely have to either change the model
equation or provide a new starting guess. This is an example of how to
interpret the results of a Colossus run.

There are additional variants of these functions which will be described
outside of this section. They return a similar list of results.
