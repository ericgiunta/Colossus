...
def Write_EXP_Event(iname,oname,paras,seed=3742621):
    tstart = time.time()
    ...
    psdf0=psdf0.withColumn("cov_rate", psdf0.cov_rate*(lit(paras[7]) + paras[8] *exp(paras[9]*psdf0.dose)))
...
for i_f in ["5050","7525","2575"]:
    for i in [1,2,4,5]:
        if i==1:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, 2, -1, 0.65/30]
        elif i==2:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6,2, -1, 0.2/30]
        elif i==4:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6,0, 1, 0.2/30]
        elif i==5:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6,0, 1, 0.8/30]
        iname="{}.parquet".format(i_f)
        oname="{}_EXP_4405{}.parquet".format(i_f,i)
        print(oname)
        if os.path.exists(oname):
            print("exists")
        else:
            pass
            Write_EXP_Event(iname,oname,paras)
