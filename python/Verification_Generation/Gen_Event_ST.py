...
def Write_ST_Event(iname,oname,paras,seed=3742621):
    tstart = time.time()
    ...
    a0=paras[7]
    y0=paras[8]
    psdf0=psdf0.withColumn("cov_0", when(psdf0.dose>y0, a0).otherwise(lit(0)))
    #
...
for i_f in ["5050","7525","2575"]:
    for i in [1,2,4,5]:
        if i==1:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6, -0.75, 10]
        elif i==2:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6,  -0.1, 5]
        elif i==4:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6,  0.1, 5]
        elif i==5:
            paras=[0.1,0.1,0.69,0.1,-0.1,-0.69,0.6,  2, 10]
        iname="{}.parquet".format(i_f)
        oname="{}_ST_4405{}.parquet".format(i_f,i)
        print(oname)
        if os.path.exists(oname):
            print("exists")
        else:
            pass
            Write_ST_Event(iname,oname,paras)
