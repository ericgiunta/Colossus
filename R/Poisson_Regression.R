...
#' Performs Poisson regression using the omnibus function with multiple outcome realizations
#'
#' \code{RunPoisRegression_Omnibus_Multioutcome} uses user provided data, time/event columns,
#'       vectors specifying the model, and options to control the convergence
#'       and starting positions. Used for 2DMC column uncertainty methods.
#'       Returns optimized parameters, log-likelihood, and standard error for each realization.
#'       Has additional options for using stratification
...
RunPoisRegression_Omnibus_Multioutcome <- function(df, pyr0 = "pyr", event0 = "event", names = "CONST", term_n = 0, tform = "loglin", keep_constant = 0, a_n = 0, modelform = "M", realization_columns = c("event0", "event1"), control = list(), strat_col = "null", model_control = list(), cons_mat = as.matrix(0), cons_vec = 0) {
  ...
  dose_all <- as.matrix(df[, dose_names, with = FALSE])
  run_size <- nrow(df)
  e <- pois_multioutcome_Omnibus_transition(
    as.matrix(df[, ce, with = FALSE]),
    term_n, tform, a_n,
    event_cols, dfc, x_all, dose_all,
    0, modelform, control,
    keep_constant, term_tot, strata_vals, as.matrix(df[, val_cols, with = FALSE]),
    model_control, cons_mat, cons_vec
  )
