...
CoxMartingale <- function(verbose, df, time1, time2, event0, e, t, ch, dnames, plot_name, age_unit, studyID) {
    ...
    for (cov_i in seq_len(length(dnames))) {
        ...
        if (studyID %in% names(df)) {
            dfr <- data.table::data.table("Risks"=e$Risks, "ch_e" = ch_e,
                "ch_s" = ch_s, "e"=e_i,
        ...
        #
        ##
        dft <- data.table::data.table("cov_max"=times$cov,
                                      "time_max"=times$time,
        ...
        if (system.file(package='ggplot2') != "") {
            g <- ggplot2::ggplot(dft, ggplot2::aes(x=.data$cov_max,
                                 y=.data$res_sum)) +
                 ggplot2::geom_point(color="black") +
                 ggplot2::labs(x=paste("Max", dname, sep = " "),
                               y="Martingale Residuals")
            ggplot2::ggsave(paste(plot_name, "_", dname, '_martin_plot.jpeg',
                                  sep = ""),
    ...
    dft <- data.table::data.table("time_max"=dfr$time, "res_sum"=dfr$res,
                                  "event"=dfr$e)
#    print(dft)
    table_out[['survival_time']] <- dft
    #
    if (system.file(package='ggplot2') != "") {
        g <- ggplot2::ggplot(dft, ggplot2::aes(x=.data$time_max,
                                               y=.data$res_sum)) +
            ggplot2::geom_point(color="black") +
            ggplot2::labs(x=paste("Max Age", sep = " "),
                          y="Martingale Residuals")
...
CoxSurvival <- function(t, h, ch, surv, plot_name, verbose, time_lims, age_unit) {
    ...
    if (system.file(package='ggplot2') != "") {
        g <- ggplot2::ggplot(data=dft, ggplot2::aes(x=.data$t, y=.data$s)) +
             ggplot2::geom_line() + ggplot2::labs(x="Log-Age",
...
CoxKaplanMeier <- function(verbose, studyID, names, df, event0, time1, time2, tu, term_n, tform, a_n, er, fir, der_iden, modelform, control, keep_constant, plot_type, age_unit, model_control = list()) {
    ...
    tu <- sort(unlist(unique(dfend[, time2, with = FALSE]),
                      use.names = FALSE)) #all event times
    #
    #
    tu_s <- c(0.0, tu)
    ...
    #
    if (system.file(package='ggplot2') != "") {
        g <- ggplot2::ggplot(data=dft, ggplot2::aes(x=.data$t_t,
                                                    y=.data$n_t)) +
            ggplot2::geom_line() +
            ggplot2::labs(x=paste("age (", age_unit, ")", sep = ""),
                          y="Survival")
    ...
    for (fir_KM in seq_len(length(names))) {
        ...
        if (system.file(package='ggplot2') != "") {
            if (length(uniq) >= 10) {
                ...
                ggplot2::ggsave(paste(plot_type[2], "_risk_plot_", fir_KM,
                                      ".jpeg", sep = ""),
                                device="jpeg", dpi="retina", width=7, height=7)
                #
            } else {
...
#        #
    ...
    if (system.file(package='ggplot2') != "") {
        g <- ggplot2::ggplot() + ggplot2::geom_point(data=dft,
             ggplot2::aes(x=.data$t, y=.data$surv, group=.data$cat_group,
                          color=.data$cat_group))
        g <- g + ggplot2::scale_colour_discrete(breaks=sbreaks, labels=slabels)
        g <- g + ggplot2::labs(x=paste("age (", age_unit, ")", sep = ""),
                               y="Survival") + ggplot2::ylim(0, 1)
        ggplot2::ggsave(paste(plot_name, '_strat_surv_plot_', strat_col,
                              '.jpeg', sep = ""),
                        device="jpeg", dpi="retina", width=7, height=7)
    }
    return (table_out)
...
PlotCox_Schoenfeld_Residual <- function(df, time1, time2, event0, names, term_n, tform, keep_constant, a_n, modelform, fir, der_iden, control, age_unit, plot_name, model_control = list()) {        
    ...
    res_list <- Plot_Omnibus_transition(term_n, tform, a_n, dfc, x_all,
                                        fir, der_iden,
                                        modelform, control,
                                        as.matrix(df[, ce, with = FALSE]),
    ...
    for (cov in seq_len(length(a_n))) {
        if (keep_constant[cov] == 0) {
            ...
            #
            if (system.file(package='ggplot2') != "") {
                g <- ggplot2::ggplot(dft, ggplot2::aes(x=.data$time,
                                                       y=.data$y)) +
                    ggplot2::geom_point(color="black") +
                    ggplot2::labs(x=paste("age (", age_unit, ")", sep = ""),
                    y=paste("Schoenfeld Residual (", names[cov], tform[cov],
                            ")", sep = " "))
                ggplot2::ggsave(paste(plot_name, "_schoenfeld_", cov_res,
                                      ".jpeg", sep = ""),
                                device="jpeg", dpi="retina", width=7, height=7)
                #
                g <- ggplot2::ggplot(dft, ggplot2::aes(x=.data$time,
                    ...
                    y=paste("Schoenfeld Residual Scaled (", names[cov],
                            tform[cov], ")", sep = " "))
                ggplot2::ggsave(paste(plot_name, "_schoenfeld_scaled_",
                                cov_res, ".jpeg", sep = ""),
                                device="jpeg", dpi="retina", width=7, height=7)
            }
            #
