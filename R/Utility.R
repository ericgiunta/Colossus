...
#' fir <- 0
#' der_iden <- 0
#' control <- list("ncores" = 2, 'lr' = 0.75, 'maxiter' = -1,
...
#' Gather_Guesses_CPP(df, dfc, names, term_n, tform, keep_constant,
    ...
    for (i in seq_len(length(a_n))) {
        ...
        if (length(term_n)<length(names)) {
            stop(paste("Error: Terms used: ", length(term_n),
                       ", Covariates used: ", length(names), sep = ""))
        ...
        if (length(keep_constant)<length(names)) {
            keep_constant <- c(keep_constant, rep(0.0,
        ...
        #
        keep <- risk_check_transition(term_n, tform, a_n0, dfc, x_all,
                                      fir, modelform,
...
#            stop()
        ...
        #
        df <- data.table::data.table("term_n"=term_n, "tform"=tform,
...
Def_model_control <- function(control) {
    names(control) <- tolower(names(control))
    control_def_names <- c('single', 'basic', 'null', 'cr',
...
factorize_par <-function(df, col_list, verbose=0, nthreads=as.numeric(detectCores())) {
    ...
    #
    df0 <- Gen_Fac_Par(as.matrix(df[, col_list, with = FALSE]), vals,
                       cols, nthreads)
    df0 <- data.table::as.data.table(df0)
    names(df0) <- names
    #
    col_keep <- Check_Dupe_Columns(df0, names, rep(0, length(cols)),
                                   verbose, TRUE)
    #
    #
...
gen_time_dep <- function(df, time1, time2, event0, iscox, dt, new_names, dep_cols, func_form, fname, tform, nthreads=as.numeric(detectCores())) {
    ...
    Write_Time_Dep(x_time, x_dep, x_same, x_event, dt, fname,
                   tform, tu, iscox, nthreads)
    df_new <- data.table::fread(fname, data.table=TRUE,
...
Date_Shift <- function(df, dcol0, dcol1, col_name, units="days") {
    ...
    df[, col_name] <- difftime(strptime(df$dt1, format = "%m-%d-%Y",
                                        tz = 'UTC'),
                   strptime(df$dt0,  format = "%m-%d-%Y"), units = units,
                            tz = 'UTC')
...
#'y0 <- c(1990, 1991, 1997, 1998)
#'y1 <- c(2001, 2003, 2005, 2006)
#'df <- data.table::data.table("m0"=m0, "m1"=m1,
#'                             "d0"=d0, "d1"=d1,
#'                             "y0"=y0, "y1"=y1)
...
System_Version <- function() {
  ...
  Rcomp <- Rcomp_version()
  OMP <- OMP_Check()
  list("Operating System"=os, "Default c++"=gcc, "R Compiler"=Rcomp,
       "OpenMP Enabled"=OMP)
}
